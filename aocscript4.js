...
var testingPassphrase1 = "aa bb cc dd ee";
var testingPassphrase2 = "aa bb cc dd aa";
var testingPassphrase3 = "aa bb cc dd aaa";

var testingPassphrases = `aa bb cc dd ee
aa bb cc dd aa
aa bb cc dd aaa`;


var inputList = `oaoe rxeq vssdqtu xrk cjv yaoqp loo
...
function parseForDuplicates(passphrase) {
	var splitPassphrase = passphrase.split(" ");

	for(var i = 0; i < splitPassphrase.length; i++) {
		for(var j = 0; j < splitPassphrase.length; j++) {
			if(i != j && splitPassphrase[i] == splitPassphrase[j]) {
				// console.log("match");
				return false;
			}
		}
...
function processListOfPassPhrases(list) {
	var listSplit = list.split("\n");

	var counter = 0;

	for (var i = 0; i < listSplit.length; i++) {
		if(parseForDuplicates(listSplit[i])) {
			counter++;
		}
	}
